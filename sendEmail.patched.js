console.log("[BOOT] sendEmail.js loaded");
...
const sendEmail = async ({ email, subject, payload, template, throwError = true }) => {
  try {
    ...
    const transporterOptions = {
      ...
      secure: String(process.env.EMAIL_ENCRYPTION || '').toLowerCase() === 'ssl' || Number(process.env.EMAIL_PORT) === 465,
      auth: {
    ...
    // Set requireTLS for STARTTLS
    if (String(process.env.EMAIL_ENCRYPTION || '').toLowerCase() === 'starttls') {
